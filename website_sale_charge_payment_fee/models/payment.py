...
class TransferPaymentTransaction(models.Model):
    ...
    def _transfer_form_validate(self, data):
        if (
            self.acquirer_id.provider == "transfer"
            and self.acquirer_id.auto_confirm == "generate_and_pay_invoice"
        ):
            _logger.info(
                "Validated transfer payment for tx %s: set as done" % (self.reference)
            )
        ...
        if (
            self.acquirer_id.provider == "transfer"
            and self.acquirer_id.auto_confirm == "confirm_so"
        ):
            _logger.info(
                "Validated transfer payment for tx %s: set as done" % (self.reference)
            )
