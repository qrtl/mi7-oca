...
def _new_get_tax_totals(self, partner, tax_lines_data, amount_total, amount_untaxed, currency):  # noqa
    account_tax = self.env['account.tax']

    grouped_taxes = defaultdict(lambda: defaultdict(lambda: {'base_amount': 0.0, 'tax_amount': 0.0, 'base_line_keys': set()}))  # noqa
    subtotal_priorities = {}
    for line_data in tax_lines_data:
        ...
        if subtotal_title not in subtotal_priorities or new_priority < subtotal_priorities[subtotal_title]:  # noqa
        ...
        if 'base_amount' in line_data:
            # Base line
            if tax_group == line_data.get('tax_affecting_base', account_tax).tax_group_id:  # noqa
            ...
            groups_vals.append(
                {
                    ...
                    'tax_group_base_amount': amounts['base_amount'],
                    'formatted_tax_group_amount': formatLang(self.env, tax_group_amount, currency_obj=currency),  # QRTL
                    'formatted_tax_group_base_amount': formatLang(self.env, amounts['base_amount'], currency_obj=currency),
                    'tax_group_id': group.id,
                    'group_key': '%s-%s' %(subtotal_title, group.id),
                }
